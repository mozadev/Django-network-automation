...
def commands_to_huawei(sub_interface, vrf_new, vrf_old, ip_interface, ip_vrf, cid_cliente, cliente, asnumber, password, new_grupo):
    vrf_new_str = convert_num_to_str(vrf_new, 5)
    ...
    commitear = [
        # Commitear
        {"prompt": "\]", "command": f"quit"},
        {"prompt": "\[Y\(yes\)\/N\(no\)\/C\(cancel\)\]:", "command": "N"},
        # Validación final
        {"prompt": "\>", "command": f"dis curr int {sub_interface}"},
    ]
